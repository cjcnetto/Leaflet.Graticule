...
L.LatLngGraticule = L.Layer.extend({
    options: {
        showLabel: true,
        drawTextFunction: (ctx, text, x, y, options)=> {
            ctx.save();
            ctx.fillStyle = 'rgba(0, 0, 0, 0.25)';
            var width = ctx.measureText(text).width;
            ctx.fillRect(x-5, y+5, width+10, -1 * (parseInt(ctx.font, 10) +10));
            ctx.fillStyle = '#FFFFFF';
            ctx.fillText(text, x-2.5, y-2.5);
            ctx.restore();
        },
        opacity: 1,
        weight: 0.8,
...
// 	},
    ...
    __draw: function(label) {
        ...
        if (L.Browser.canvas && map) {
            ...
            _lon_r = _lon_r + _point_per_lon;
            _lon_l = _lon_l - _point_per_lon;
            ...
            function __draw_lat_line(self, lat_tick) {
                ...
                if (curvedLat) {
                    ...
                    if (self.options.showLabel && label) {
                        ...
                        if(self.options.drawTextFunction){
                            self.options.drawTextFunction(ctx, latstr, 0, _yy, self.options);
                            self.options.drawTextFunction(ctx, latstr, ww-txtWidth-2.5, _yy, self.options);
                        }else {
                            ctx.fillText(latstr, 0, _yy);
                            ctx.fillText(latstr, ww-txtWidth-2.5, _yy);
                        }
                    }
                }
            };
            ...
            function __draw_lon_line(self, lon_tick) {
                ...
                if (curvedLon) {
                    ...
                    if (self.options.showLabel && label) {
                        ...
                        if(self.options.drawTextFunction){
                            self.options.drawTextFunction(ctx, lngstr, tt.x - (txtWidth/2), txtHeight+1, self.options);
                            self.options.drawTextFunction(ctx, lngstr, bb.x, hh-3, self.options);
                        }else{
                            ctx.fillText(lngstr, tt.x - (txtWidth/2), txtHeight+1);
                            ctx.fillText(lngstr, bb.x, hh-3);
                        }
                    }
                }
            };
